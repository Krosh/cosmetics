...
class LinkBackendController extends yupe\components\controllers\BackController
{
    ...
    public function actionLinkIngridient()
    {
        $r = Yii::app()->getRequest();
        if ($r->isPostRequest) {
            $product_id = $r->getParam('product_id');
            $ingredient_id = $r->getParam('ingredient_id');

            $product = Product::model()->findByPk($product_id);
            $ingredient = Ingredient::model()->findByPk($ingredient_id);
            if ($product && $ingredient) {
                Yii::app()->ajax->raw(['result' => $product->linkWithIngredient($ingredient->id)]);
            }
        }
    }
    ...
    {
        if (Yii::app()->getRequest()->getIsPostRequest()) {
           ProductIngredient::model()->findByPk($id)->delete();
        } else {
            throw new CHttpException(400, Yii::t('StoreModule.store', 'Unknown request. Don\'t repeat it please!'));
        }
    }

    public function actionIngredientIndex()
    {
        $model = new ProductIngredient('search');
        $model->unsetAttributes();
        $model->attributes = Yii::app()->getRequest()->getParam('ProductIngredient');
        $this->render('/productBackend/_ingredients_form', ['searchModel' => $model, 'product' => new Product()]);
    }
