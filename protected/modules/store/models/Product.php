...
class Product extends yupe\models\YModel implements ICommentable
{
    ...
    public function relations()
    {
        return [
            ...
            'ingredientsRelation' => [self::HAS_MANY, 'ProductIngredient', 'product_id'],
            'ingredients' => [self::HAS_MANY, 'Ingredient', ['category_id' => 'id'], 'through' => 'ingredientsRelation'],
    ...
    public function linkWithIngredient($ingredient)
    {
        $link = new ProductIngredient();
        $link->product_id = $this->id;
        $link->ingredient_id = $ingredient;
    ...
    public function getPackProductDataProvider($typeCode = null)
    {
        // TODO:: переписать для улучшения производительности
        $inPackAttributes = [1,2,3,4];
        $criteria = new CDbCriteria();
        $criteria->addInCondition("attribute_id",$inPackAttributes);
        $criteria->compare("number_value",1);
        $criteria->compare("product_id",$this->id);
        $attrs = AttributeValue::model()->findAll($criteria);
        $ids = array();
        foreach ($attrs as $item)
        {
            $ids[] = $item->attribute_id;
        }
        $criteria = new CDbCriteria();
        $criteria->addInCondition("attribute_id",$ids);
        $criteria->compare("number_value",1);
        $attrs = AttributeValue::model()->findAll($criteria);
        $ids = [];
        foreach ($attrs as $item)
     ...
     * @return CActiveDataProvider
     */
    public function getLinkedIngridientsDataProvider()
    {
        $ids = [];
        foreach ($this->ingredientsRelation as $item)
        {
            $ids[] = $item->ingredient_id;
        }
        $criteria = new CDbCriteria();
        $criteria->addInCondition("id",$ids);
