...
<div class="main__order-box grid">
    ...
    <div class="order-box">
                ...
                <div class="order-box__body">
                    <?php foreach ((array)$model->delivery->paymentMethods as $payment): ?>
                        <div class="rich-radio payment-method">
                            <input class="rich-radio__input payment-method-radio" type="radio" name="payment_method_id"
                                   value="<?= $payment->id; ?>" checked="" id="payment-<?= $payment->id; ?>"
                                   hidden="hidden">
                                ...
                                <div class="rich-radio-body">
                                    <div class="rich-radio-body__content">
                                        <div class="rich-radio-body__heading">
                                            <span
                                                class="rich-radio-body__title"><?= CHtml::encode($payment->name); ?></span>
                                        </div>
                                    </div>
                                </div>
                            </label>
        ...
        <div class="order-box__header order-box__header_black">
            <div class="order-box__header-content"><?= Yii::t("OrderModule.order", "Order #"); ?>
                <b><?= $model->id ?></b> (<?= $model->status->getTitle(); ?>)
            </div>
        </div>
        <div class="order-box__body">
            ...
            <?php if (!empty($model->payment_method_id)): ?>
                ...
                <?php if ($model->payment_method_id == 4) : ?>
                    <div class="detail-view">
                        <div class="detail-view__item">
                            Ваш заказ принят, ближайшее время с Вами свяжется оператор для уточнения стоимости и условий
                            доставки. Спасибо, что выбрали органическую косметику ручной работы «АлтайЯ»
                        </div>
                    </div>
                <?php endif; ?>
            <?php endif; ?>
    ...
    <div class="order-box">
            ...
            <?php foreach ((array)$model->products as $position): ?>
                <div class="cart-list__item js-cart__item">
                    <div class="cart-item js-cart__item">
                        ...
                        <div class="cart-item__price"><?= number_format($position->price, 0, '.', ' '); ?><span
                                class="ruble"> <?= Yii::t("OrderModule.order", Yii::app()->getModule('store')->currency); ?></span>
                        </div>
                        <div class="cart-item__quantity"><?= $position->quantity; ?></div>
                        <div class="cart-item__summ"><?= number_format($position->getTotalPrice(), 0, '.', ' '); ?><span
                                class="ruble"> <?= Yii::t("OrderModule.order", Yii::app()->getModule('store')->currency); ?></span>
                        </div>
                    </div>
                </div>
        ...
        <div class="order-box__bottom">
            <div class="order-box__subtotal"><?= Yii::t("OrderModule.order", "Total"); ?>:
                <div class="product-price"><?= number_format($model->total_price, 0, '.', ' '); ?><span
                        class="ruble"> <?= Yii::t("OrderModule.order", Yii::app()->getModule('store')->currency); ?></span>
                </div>
            </div>
            <div class="order-box__subtotal"><?= Yii::t("OrderModule.order", "Delivery price"); ?>:
                <div class="product-price"><?= number_format($model->getDeliveryPrice(), 0, '.', ' '); ?><span
                        class="ruble"> <?= Yii::t("OrderModule.order", Yii::app()->getModule('store')->currency); ?></span>
                </div>
            </div>
            <?php
            ...
            <?php if ($couponDiscount > 0): ?>
                <div class="order-box__subtotal">Скидка по промокоду:
                    <div class="product-price"><?= number_format($couponDiscount, 0, '.', ' ') ?><span
                            class="ruble"> <?= Yii::t("OrderModule.order", Yii::app()->getModule('store')->currency); ?></span>
                    </div>
                </div>
            <?php endif; ?>
            <div class="order-box__subtotal order-box__subtotal_big"><?= Yii::t("OrderModule.order", "Total"); ?>:
                <div class="product-price"><?= number_format($model->getTotalPriceWithDelivery(), 0, '.', ' '); ?><span
                        class="ruble"> <?= Yii::t("OrderModule.order", Yii::app()->getModule('store')->currency); ?></span>
                </div>
            </div>
        </div>
